...
/**
 * Forge's `ModConfigEvent.Loading` and `ModConfigEvent.Reloading` events, adapted for Fabric's callback event style.
 *
 * @author Fuzs
 */
public final class ModConfigEvent {
	 ...
	 * <p>
	 * IMPORTANT: Unlike on Forge there are is no mod event bus for firing mod specific events on Fabric,
	 * meaning this event is fired for every mod for every config.
	...
	public static final Event<Loading> LOADING = EventFactory.createArrayBacked(Loading.class, listeners -> config -> {
		for (Loading event : listeners) event.onModConfigLoading(config);
	});

	/**
	 * Called when a config is reloaded which happens when the file is updated by ConfigWatcher and when it is synced
	 * from the server.
	 * <p>
	 * IMPORTANT: Unlike on Forge there are is no mod event bus for firing mod specific events on Fabric,
	 * meaning this event is fired for every mod for every config.
	...
	public static final Event<Reloading> RELOADING = EventFactory.createArrayBacked(Reloading.class, listeners -> config -> {
		for (Reloading event : listeners) event.onModConfigReloading(config);
	});
